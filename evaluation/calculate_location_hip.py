#!/usr/bin/env python3
"""
Hip-Only Badminton Position Tracker

Uses only hip joint positions with simple homography transformation.
No height corrections or complex calculations.

Usage: python hip_only_tracker.py <video_file_path> [--debug]
"""
...
class HipOnlyTracker:
    """Tracker using only hip joint positions."""
    ...
    def calculate_player_position(self, joints: List[Dict]) -> Optional[Dict[str, float]]:
        ...
        world_x, world_y = self.transform_point_to_world(hip_center)

        # For consistency with other methods, set ankle positions to (0,0)
        return {
            'hip_world_X': world_x,
            'hip_world_Y': world_y,
            'left_ankle_world_X': 0.0,
            'left_ankle_world_Y': 0.0,
            'right_ankle_world_X': 0.0,
            'right_ankle_world_Y': 0.0,
    ...
    def save_results(self) -> None:
        ...
        output_data = {
            ...
            'tracking_method': "Hip-only positioning with simple homography transformation",
            'processing_info': {
                'total_positions': len(self.player_positions),
                'frames_with_players': frames_with_players,
                'method_details': {
                    'hip_joints_only': True,
    ...
    def run(self) -> None:
        ...
        print(f"Starting hip-only tracking for: {self.video_name}")
        ...
        try:
            ...
            self.save_results()
            print("Hip-only tracking completed successfully!")
...
def main():
    """Main function."""
    if len(sys.argv) < 2:
        print("Usage: python hip_only_tracker.py <video_file_path> [--debug]")
        sys.exit(1)
    ...
    tracker = HipOnlyTracker(video_path, debug=debug)
    tracker.run()
